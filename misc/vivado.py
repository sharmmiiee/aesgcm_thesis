...
class XilinxVivadoToolchain(GenericToolchain):
    ...
    def build_project(self):
        ...
        ## Timing path analysis (get the delay path of the riscv_mult_custom.v - sharmine)
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {{NAME == op_a_i}}] -to [get_pins -hierarchical -filter {{NAME == result_h_o}}] -setup")
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {{NAME == op_a_i[*]}}] -to [get_pins -hierarchical -filter {{NAME == result_h_o[*]}}] -setup")
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {{NAME =~ *op_a_i*}}] -to [get_pins -hierarchical -filter {{NAME =~ *result_h_o*}}] -setup")
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {{NAME =~ *op_b_i*}}] -to [get_pins -hierarchical -filter {{NAME =~ *result_h_o*}}] -delay_type max")
        #tcl.append("get_timing_paths -from riscv_mult_custom/op_a_i[*] -to riscv_mult_custom/result_h_o[*] -setup")
        #tcl.append("get_timing_paths -from [get_pins riscv_mult_custom/op_a_i] -to [get_pins riscv_mult_custom/result_h_o] -setup")
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {NAME == op_a_i}] -setup")
        #tcl.append("get_timing_paths -from [get_pins -hierarchical -filter {NAME == op_a_i} -of_objects [get_cells -hierarchical -filter {NAME == riscv_mult_custom}]] -to [get_pins -hierarchical -filter {NAME == result_h_o} -of_objects [get_cells -hierarchical -filter {NAME == riscv_mult_custom}]]")
        #tcl.append("report_timing -from [get_ports op_a_i ] -to [get_cells -hier -filter  {REF_NAME =~  result_h_o*}]  -name test   -max_paths 100")
        #tcl.append("get_timing_paths -max_paths 10000 -setup")


        #tcl.append(f"report_timing_summary -datasheet -setup -max_paths 1000 -file {self._build_name}_timing_sharmine.rpt")
